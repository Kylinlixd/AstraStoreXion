...
- [ ] **Step 1: Test the DiskStore threshold and capacity shape.**
...
- [ ] **Step 2: Test API capacity and HTTP 507.**
...
- [ ] **Step 3: Test `xionctl capacity` calls the authenticated endpoint and prints JSON.**
...
- [ ] **Step 4: Add `/api/v1/capacity` and error mapping.**
...
- [ ] **Step 1: Implement `xionctl capacity` and usage text.**
- [ ] **Step 2: Document `XION_STORAGE_PAUSE_AT_PERCENT=90` and the automatic behavior.**
- [ ] **Step 3: Build Linux amd64, install to `/usr/local/bin/xionctl`, and run `xionctl capacity` on the server.**
